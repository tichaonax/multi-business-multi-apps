...
export default function GroceryStorePage() {
  ...
  const canViewSuppliers = isSysAdmin || hasBizPermission('canViewSuppliers') || hasBizPermission('canAccessFinancialData')
  const canManageEmployees = isSysAdmin || hasBizPermission('canManageEmployees') || hasBizPermission('canEditEmployees')

  // Redirect salespersons (and anyone without management access) straight to POS
  useEffect(() => {
    if (permLoading) return
    if (!canViewFinancials && !canManageInv && !canViewInvReports) {
      router.replace('/grocery/pos')
    }
  ...
  useEffect(() => {
    if (!effectiveBusinessId || !canViewFinancials) {
    ...
    loadData()
  }, [effectiveBusinessId, canViewFinancials])

  // Show nothing while permissions load (prevents flash of restricted content)
  if (permLoading) {
    return (
  ...
  // Non-managers are redirected; this renders briefly before the redirect fires
      ...
      <BusinessTypeRoute requiredBusinessType="grocery">
        ...
        >
          <div className="space-y-6">
            {/* Inventory Overview Widget — only for inventory managers */}
            ...
            {canViewFinancials && effectiveBusinessId && (
            ...
            {/* Key Metrics — financial data, managers only */}
            {canViewFinancials && (
            ...
            {/* Advanced Features — managers only */}
            {canViewFinancials && (
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 border border-green-200 dark:border-green-800 rounded-lg p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="card p-4 border border-green-200 dark:border-green-800">
                    ...
                    <p className="text-sm text-green-700 dark:text-green-300">FIFO rotation, automatic markdowns, waste tracking, and donation management.</p>
                  </div>
                  <div className="card p-4 border border-green-200 dark:border-green-800">
                    <h3 className="font-semibold text-green-900 dark:text-green-100 mb-2">⚖️ Weight-Based Pricing</h3>
                    <p className="text-sm text-green-700 dark:text-green-300">Integrated scales, PLU codes, tare weights, and price-per-pound calculations.</p>
                  </div>
                  <div className="card p-4 border border-green-200 dark:border-green-800">
                    <h3 className="font-semibold text-green-900 dark:text-green-100 mb-2">🌡️ Cold Chain Monitoring</h3>
                    <p className="text-sm text-green-700 dark:text-green-300">Temperature logging, alert systems, compliance reporting, and quality tracking.</p>
                  </div>
                  <div className="card p-4 border border-green-200 dark:border-green-800">
                    <h3 className="font-semibold text-green-900 dark:text-green-100 mb-2">🎯 Loyalty Programs</h3>
                    <p className="text-sm text-green-700 dark:text-green-300">Points systems, targeted promotions, purchase history, and customer segmentation.</p>
                  </div>
                </div>
              </div>
            )}

            {/* Department Status — managers only */}
            {canViewFinancials && (
              <div className="card p-6">
