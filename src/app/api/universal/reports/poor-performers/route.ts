...
export async function GET(request: NextRequest) {
  try {
    ...
    // Search is applied here, server-side, against the FULL catalog before
    // pagination — not just the rows that happen to already be on the
    // current page, so a match on page 3 is findable while looking at page 1.
    const [settingsRow, products, sales, movements, business] = await Promise.all([
      ...
      getUnifiedProducts({ businessId, search: search ?? undefined }),
