...
/**
 * POST /api/esp32/[deviceId]/mac-whitelist
 * Add MAC addresses from a token to the whitelist (VIP bypass)
 *
 * Body:
 * - token: WiFi token to extract MACs from
 * - note: Note about whitelist entry (optional, max 31 chars)
...
export async function POST(
  ...
  try {
    ...
    const { token, note = 'VIP access', businessId } = body
    ...
    const response = await fetch(`${integration.apiUrl}/api/mac/whitelist`, {
      method: 'POST',
      headers: {
    ...
    if (!data.success) {
      return NextResponse.json(
        { error: data.error || 'Failed to add to whitelist' },
        { status: 400 }
      )
    }
    ...
    console.error('Error adding to ESP32 whitelist:', error)
    return NextResponse.json(
