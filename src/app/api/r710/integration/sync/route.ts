...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[R710 Sync] SSID changed: ${wlanRecord.ssid} → ${deviceWlan.ssid}`);

    // Update database with current device configuration
    const updatedWlan = await prisma.r710Wlans.update({
      where: { id: wlanRecord.id },
      data: {
        ssid: deviceWlan.ssid,
        // Update other fields if they exist in device response
        ...(deviceWlan.valid && { validDays: parseInt(deviceWlan.valid) }),
        updatedAt: new Date()
      }
    });

    console.log(`[R710 Sync] Database updated successfully`);

    return NextResponse.json({
      message: 'WLAN configuration synced successfully',
      changed: true,
      previousSsid: wlanRecord.ssid,
      currentSsid: deviceWlan.ssid,
      wlan: {
