...
export async function GET(
  ...
  try {
    ...
    const depositSearchFilter: any = search
      ? {
          OR: [
            { manualNote: { contains: search, mode: 'insensitive' } },
            { autoGeneratedNote: { contains: search, mode: 'insensitive' } },
            { sourceType: { contains: search, mode: 'insensitive' } },
            { sourceBusiness: { name: { contains: search, mode: 'insensitive' } } },
            { depositSource: { name: { contains: search, mode: 'insensitive' } } },
            { fundSource: { name: { contains: search, mode: 'insensitive' } } },
    ...
    const paymentSearchFilter: any = search
      ? {
          OR: [
            { notes: { contains: search, mode: 'insensitive' } },
            { receiptNumber: { contains: search, mode: 'insensitive' } },
            { payeeUser: { name: { contains: search, mode: 'insensitive' } } },
    ...
    deposits.forEach((deposit) => {
      ...
      } else if (deposit.sourceType === 'BUSINESS' && deposit.autoGeneratedNote?.startsWith('EOD batch approval')) {
        category = {
          id: 'eod-payment-funding',
      ...
      // For auto-transfer deposits the autoGeneratedNote already describes the source well
      const depositDescription = isAutoXferIn
        ? (deposit.autoGeneratedNote || 'Auto Expense Transfer In')
        : description
      // Parse the source account name from the auto-generated note, e.g.
      // 'Auto-transfer from "Fashions HXI Expense Account" for supplier payment #abc'
      const autoTransferSource = isAutoXferIn
    ...
    payments.forEach((payment) => {
      ...
      } else if ((payment as any).payeeSupplier) {
        payeeName = (payment as any).payeeSupplier.name
      }

      const isAutoXferOut = (payment as any).paymentType === 'TRANSFER_OUT'
      let description: string
      if (isAutoXferOut) {
        // Use the notes which contain 'Auto-transfer to primary account for supplier payment #...'
        description = (payment as any).notes || 'Auto Expense Transfer Out'
      } else {
