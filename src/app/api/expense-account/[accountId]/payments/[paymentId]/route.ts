...
export async function PATCH(
  ...
  try {
    ...
    if (body.action === 'markPaid') {
      ...
      // Plain CASH payment: mirror the EcoCash branch above exactly, but
      // against the CASH-channel pool. The account's own balance already
      // reflects this payment as committed from the moment it was
      // APPROVED (see updateExpenseAccountBalanceTx) — that's a budget
      // check, not a liquidity check. This is the moment real physical
      // cash actually leaves the till, regardless of what originally
      // funded the account, so it needs its own independent check against
      // real cash on hand, same as EcoCash needs its own wallet check.
      if (!businessId) {
        return NextResponse.json({ error: 'Cannot resolve business for this payment' }, { status: 400 })
      }
      const cashAmount = Number(existingPayment.amount)

      const cashBucketAgg = await prisma.cashBucketEntry.groupBy({
        by: ['direction'],
        where: { businessId, paymentChannel: 'CASH' },
        _sum: { amount: true },
      })
      const cashInflow = Number(cashBucketAgg.find((r: any) => r.direction === 'INFLOW')?._sum?.amount ?? 0)
      const cashOutflow = Number(cashBucketAgg.find((r: any) => r.direction === 'OUTFLOW')?._sum?.amount ?? 0)
      const cashBalance = cashInflow - cashOutflow
      if (cashBalance < cashAmount) {
        return NextResponse.json(
          { error: `Insufficient cash on hand. Available: $${cashBalance.toFixed(2)}, Required: $${cashAmount.toFixed(2)}.` },
          { status: 400 }
        )
      }
      ...
      const updated = await prisma.$transaction(async (tx: any) => {
        await tx.expenseAccountDeposits.create({
          data: {
            ...
            amount: cashAmount,
            depositDate: paidAt,
            autoGeneratedNote: 'Cash payment sent',
            createdBy: user.id,
          },
        })
        await tx.cashBucketEntry.create({
