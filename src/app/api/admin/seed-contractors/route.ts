...
export async function POST(request: NextRequest) {
  ...
  if (!confirmText.startsWith('SEED-CONTRACTORS-')) return NextResponse.json({ error: 'Invalid confirmation text' }, { status: 400 })

  try {
    const result = await runScript('scripts/seed-contractors-demo.js')
    if (result.code !== 0) {
