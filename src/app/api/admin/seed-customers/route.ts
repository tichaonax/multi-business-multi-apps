import { NextRequest, NextResponse } from 'next/server'
import { getServerUser } from '@/lib/get-server-user'

const { seedCustomers } = require('../../../../../scripts/seed-demo-customers-all-businesses.js')

export async function POST(request: NextRequest) {
  const body = await request.json().catch(() => ({}))
  const devBypass = process.env.FORCE_ADMIN_SESSION === 'true' && body._forceAdmin
  if (!devBypass) {
    const user = await getServerUser()
    const currentUser = user as any
    if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    const isAdmin = currentUser?.role === 'admin' || currentUser?.isAdmin
    if (!isAdmin) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  }

  const confirmed = !!body.confirm
  const confirmText = typeof body.confirmText === 'string' ? body.confirmText : undefined
  if (!confirmed || !confirmText) return NextResponse.json({ error: 'Confirmation required' }, { status: 400 })
  if (!confirmText.startsWith('SEED-CUSTOMERS-')) return NextResponse.json({ error: 'Invalid confirmation text' }, { status: 400 })

  try {
    await seedCustomers()
    return NextResponse.json({ success: true })
  } catch (err: any) {
    console.error('Seed customers failed:', err)
    return NextResponse.json({ error: 'Failed to seed customers', message: err?.message || String(err) }, { status: 500 })
  }
}
