...
export async function GET(request: NextRequest) {
  try {
    ...
    const where: any = { businessType: 'restaurant' }
    ...
    const restaurantCategories = await prisma.businessCategories.findMany({
      where: { businessType: 'restaurant', domainId: { not: null } },
      select: {
        domainId: true,
