/**
 * HTTP-Based Full Sync - Transfer ALL System Data
 * Fallback method using record-by-record HTTP transfers
...
/**
 * Perform HTTP-based full sync (fallback method)
 * Note: Only supports PUSH direction (local → remote)
 */
export async function performHttpTransfer(
  sessionId: string,
  action: 'pull' | 'push',
  sourceNodeId: string,
  targetPeer: any,
  ...
  // HTTP method only supports PUSH for now
  if (action === 'pull') {
    throw new Error('HTTP method does not support PULL. Use backup method instead.')
  }
