...
export async function POST(request: NextRequest) {
  ...
  if (!confirmText.startsWith('UNSEED-HARDWARE-') && !confirmText.startsWith('UNSEED-')) return NextResponse.json({ error: 'Invalid confirmation text' }, { status: 400 })
  ...
  try {
    ...
    console.error('Unseed hardware failed:', err)
    return NextResponse.json({ error: 'Failed to unseed hardware demo', message: err?.message || String(err) }, { status: 500 })
  }
}
