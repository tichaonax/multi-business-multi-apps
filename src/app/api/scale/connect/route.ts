...
export async function POST(request: NextRequest) {
  ...
  if (!config || !config.comPort) {
    return NextResponse.json({ error: 'No scale configured for this business yet' }, { status: 404 })
  }

  return dispatchScaleJob(config.workstationAgentId, 'SCALE_CONNECT', { comPort: config.comPort, baudRate: config.baudRate ?? undefined }, user.id)
}
