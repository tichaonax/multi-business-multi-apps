 ...
 *
 * Body:
 * - sourceExpenseAccountId?: string  — debit an expense account (new)
 * - sourceBusinessId?: string        — debit a business account (existing)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (sourceExpenseAccountId) {
      ...
      const depositNote =
        note || `💵 Payroll funding from ${expenseAccount.accountName} (${expenseAccount.accountNumber})`
    ...
    // ── Path B: From Business Account (existing behavior) ───────────────────
    if (!sourceBusinessId) {
      return NextResponse.json(
