...
/**
 * GET /api/payroll/periods/[periodId]/reports/nssa
 * Returns NSSA employee contribution data. NSSA employer = NSSA employee (matching contribution).
 */
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const rows = slips.map((s) => {
      const nssaEmployee = Number(s.nssaEmployee || 0)
      return {
        ...
        nssaEmployee,
        nssaEmployer: nssaEmployee, // employer matches employee
      }
    })
    ...
    const totals = {
      ...
      nssaEmployer: rows.reduce((sum, r) => sum + r.nssaEmployer, 0),
    }
    ...
    console.error('Error generating NSSA report:', error)
    return NextResponse.json({ error: 'Failed to generate NSSA report' }, { status: 500 })
  }
}
