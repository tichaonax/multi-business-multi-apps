...
async function reconcilePayrollLoanDeductions(periodId: string, userId: string) {
  // Get ALL entries — use the loan's actual installment as authoritative,
      ...
      if (isPayrollLoan) {
        ...
        const deposit = await tx.payrollAccountDeposits.create({
          data: {
            ...
            autoGeneratedNote: `📥 Payroll loan deduction — ${activeLoan.recipientEmployee?.fullName ?? 'Employee'} (${activeLoan.loanNumber})`,
            createdBy: userId,
          },
        })
  ...
  const employeeId = entry.employeeId || undefined
    ...
    if (period && period.payroll_entries && period.payroll_entries.length > 0) {
      ...
      try {
        ...
        // current loan installment — even before the user has clicked "sync".
        ...
        enrichedEntries = await Promise.all(enrichedEntries.map(async entry => {
          ...
          // Apply persisted overrides — but still enforce month restriction so benefits
        ...
        // Entries still have contract attached from Step 1 — client can use pdfGenerationData as fallback
      }
      ...
      const result: any = {
        ...
        // Use snake_case key to match Prisma relation name — overrides the un-enriched
