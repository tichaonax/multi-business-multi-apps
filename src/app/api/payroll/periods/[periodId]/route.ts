 ...
 * to the linked expense account, record a repayment, and decrement the loan balance.
 */
async function reconcilePayrollLoanDeductions(periodId: string, userId: string) {
  const entries = await prisma.payrollEntries.findMany({
    ...
    select: { id: true, employeeId: true, loanDeductions: true },
  })

  for (const entry of entries) {
    if (!entry.employeeId) continue
    const deductionAmount = Number(entry.loanDeductions)
    ...
    const newBalance = Math.max(0, Number(activeLoan.remainingBalance) - deductionAmount)
    ...
    await prisma.$transaction(async (tx: any) => {
      // Create deposit to the expense account
