...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create portal client and disable token
    ...
    const result = await portalClient.disableToken({ token })

    if (!result.success) {
      return NextResponse.json(
        { error: result.error || 'Failed to disable token on portal' },
        { status: 500 }
      )
    }
    ...
    return NextResponse.json({
      success: true,
      message: 'Token disabled successfully on portal',
    })

  } catch (error: any) {
    console.error('Error disabling portal token:', error)
    return NextResponse.json(
      { error: error.message || 'Failed to disable portal token' },
      { status: 500 }
    )
  }
}
