  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      const labelHtmls = selected.map(bale => {
        ...
        return buildLabelHtml(bale, svgEl.outerHTML, businessName)
      })
      ...
      printWindow.document.write(`<!DOCTYPE html><html><head><title>Bale Barcodes</title><style>body{margin:10mm;background:white;font-family:sans-serif;}@media print{body{margin:5mm;}}</style></head><body>${rows.join('')}<script>window.onload=()=>{window.print();window.close();}<\/script></body></html>`)
