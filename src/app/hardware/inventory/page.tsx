...
function HardwareInventoryContent() {
  ...
  useEffect(() => {
    fetch('/api/admin/seed-categories?businessType=hardware')
      .then(r => r.json())
  ...
  const handleSeedCategories = async () => {
    ...
    try {
      const res = await fetch('/api/admin/seed-categories', {
        ...
        body: JSON.stringify({ businessType: 'hardware' }),
      })
    ...
    <BusinessProvider businessId={businessId}>
            ...
            <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[70]">
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8 max-w-sm w-full mx-4">
                <div className="flex flex-col items-center">
                  <div className="animate-spin rounded-full h-16 w-16 border-b-4 border-orange-600 dark:border-orange-400 mb-4"></div>
      ...
      {showMergeModal && currentBusinessId && (
        <MergeInventoryModal
          ...
          onMerged={() => { setShowMergeModal(false); setMergeMode(false); setSelectedMergeItems(new Map()); setRefreshKey(k => k + 1) }}
        />
      )}
      </BusinessTypeRoute>
    </BusinessProvider>
