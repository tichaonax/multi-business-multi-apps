...
export default function ExternalClockInPage() {
  ...
  const printIdCard = (person: Person) => {
    ...
    setTimeout(() => {
      ...
      printWindow.document.write(`<!DOCTYPE html><html><head><title>ID Card — ${person.fullName}</title><style>${styles}html,body{height:100%;margin:0;padding:0;}body{display:flex;flex-direction:column;align-items:center;padding:16px;}.print-toolbar{display:flex;align-items:center;gap:12px;margin-bottom:16px;padding:10px 16px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;width:100%;max-width:700px;box-sizing:border-box;}.print-btn{background:#1f2937;color:#fff;border:none;border-radius:6px;padding:8px 20px;font-size:14px;font-weight:600;cursor:pointer;}.print-btn:hover{background:#374151;}.print-title{font-size:13px;color:#64748b;}.card-pair{display:inline-flex;align-items:flex-start;}.fold-guide{width:0;align-self:stretch;border-left:2px dashed #888;}@media print{.print-toolbar{display:none;}body{padding:5mm;}.fold-guide{border-left-color:#bbb;}}</style></head><body><div class="print-toolbar"><button class="print-btn" onclick="window.print()">🖨️ Print / Save as PDF</button><span class="print-title">ID Card — ${person.fullName}</span></div><div class="card-pair">${cardHtml}<div class="fold-guide"></div>${cardHtml}</div></body></html>`)
