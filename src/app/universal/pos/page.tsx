...
export default function UniversalPOS() {
  ...
  // Copies every successfully-generated token's code from a section in one
  // go — comma-separated when there's more than one, so a cashier reading
  ...
  // that was clicked, not a toast() — this modal renders at z-50 and a
  // toast landed silently invisible behind it in practice, so this is
  // guaranteed visible regardless of any toast provider's stacking order.
                ...
                <div className="space-y-4 py-4">
                  ...
                  {pendingReceiptData.wifiTokens && pendingReceiptData.wifiTokens.length > 0 && (
                    <div className="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-lg border-2 border-purple-200 dark:border-purple-700">
                      <div className="flex items-center justify-between mb-3">
                        <h3 className="font-semibold text-purple-700 dark:text-purple-300 flex items-center gap-2">📶 WiFi Access Tokens</h3>
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            onClick={() => copyTokensToClipboard((pendingReceiptData.wifiTokens || []).map((t: any) => t.tokenCode).filter(Boolean), 'wifi')}
                  ...
                  {pendingReceiptData.r710Tokens && pendingReceiptData.r710Tokens.length > 0 && (
                    <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border-2 border-blue-200 dark:border-blue-700">
                      <div className="flex items-center justify-between mb-3">
                        <h3 className="font-semibold text-blue-700 dark:text-blue-300 flex items-center gap-2">📶 R710 WiFi Access</h3>
                        <div className="flex items-center gap-2">
                          <span
                          ...
                          <button
                            onClick={() => copyTokensToClipboard((pendingReceiptData.r710Tokens || []).map((t: any) => t.password).filter(Boolean), 'r710')}
