...
export function CustomBulkReportModal({ isOpen, onClose, businessId, businessName, itemId, itemName }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 p-2 print:p-0 print:bg-white print:inset-auto print:fixed-none">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-5xl max-h-[92vh] flex flex-col print:shadow-none print:rounded-none print:max-h-none print:max-w-none print:w-full">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-700 shrink-0 print:hidden">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3 py-1.5 text-xs border border-gray-300 dark:border-gray-600 rounded-lg text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
              🖨 Print
            </button>
        ...
        {/* Print-only header */}
        <div className="hidden print:block px-4 py-2 border-b border-gray-300">
          <h1 className="text-base font-bold">Custom Bulk Products — Activity Report</h1>
          {businessName && <p className="text-xs text-gray-600">{businessName}</p>}
          <p className="text-xs text-gray-500">Period: {from} to {to}</p>
        </div>
        ...
        <div className="overflow-y-auto flex-1 print:overflow-visible">
          ...
          {!loading && !error && data && data.items.length > 0 && (
            <table className="w-full text-xs print:text-[9px]">
              <thead className="bg-gray-50 dark:bg-gray-700/50 text-gray-500 dark:text-gray-400 uppercase sticky top-0 print:static">
                <tr>
                  ...
                  <th className="px-3 py-2 text-center font-medium print:hidden">Details</th>
                </tr>
              </thead>
