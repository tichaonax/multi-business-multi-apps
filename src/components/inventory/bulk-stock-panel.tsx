...
interface BulkStockRow {
  ...
  // Audited system quantity override
  systemQuantityOverride: number | null
  systemOverrideReason: string
  systemOverriddenBy: string
  systemOverriddenAt: string
...
export function BulkStockPanel({ businessId, businessName, businessType, onClose, initialMode }: BulkStockPanelProps) {
  ...
  // System-quantity override modal state
  const [overrideModalRowId, setOverrideModalRowId] = useState<string | null>(null)
  const [overrideQtyInput, setOverrideQtyInput] = useState('')
  ...
  const handleOpenOverride = (rowId: string) => {
    const row = rows.find(r => r.rowId === rowId)
    if (!row) return
    setOverrideModalRowId(rowId)
    setOverrideQtyInput(row.systemQuantityOverride !== null ? String(row.systemQuantityOverride) : row.currentStock !== null ? String(row.currentStock) : '')
    setOverrideReasonInput(row.systemOverrideReason)
  }

  const handleConfirmOverride = () => {
    if (!overrideModalRowId) return
    const qty = parseInt(overrideQtyInput, 10)
    if (isNaN(qty) || qty < 0) return
    if (!overrideReasonInput.trim()) return
    const row = rows.find(r => r.rowId === overrideModalRowId)
    if (!row) return
    updateRow(overrideModalRowId, {
      systemQuantityOverride: qty,
      systemOverrideReason: overrideReasonInput.trim(),
      systemOverriddenBy: session?.user?.email ?? session?.user?.name ?? 'unknown',
      systemOverriddenAt: new Date().toISOString(),
    })
    setOverrideModalRowId(null)
    setOverrideQtyInput('')
  ...
  const saveDraft = async () => {
    ...
    try {
      const items = rows.map((r, i) => ({
        ...
        systemQuantityOverride: r.systemQuantityOverride ?? undefined,
        systemOverrideReason: r.systemOverrideReason || undefined,
        systemOverriddenBy: r.systemOverriddenBy || undefined,
        systemOverriddenAt: r.systemOverriddenAt || undefined,
        displayOrder: i,
      }))
    ...
    try {
      ...
      const restoredRows: BulkStockRow[] = sortedItems.map((item: any) => {
        ...
        return makeRow({
          ...
          systemQuantityOverride: item.systemQuantityOverride ?? null,
          systemOverrideReason: item.systemOverrideReason ?? '',
          systemOverriddenBy: item.systemOverriddenBy ?? '',
          systemOverriddenAt: item.systemOverriddenAt ?? '',
    ...
    <div className="fixed inset-0 z-50 bg-white dark:bg-gray-900 flex flex-col pb-20">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 shrink-0 flex-wrap">
        ...
        <div className="relative flex-1 min-w-[180px]">
          ...
          {!scanLoading && !draftCheckLoading && scanInput && (
            <button
        ...
        <div className="flex items-center gap-2 px-4 py-2 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shrink-0">
          ...
          <div className="relative flex-1">
            <input
              ...
              className="w-full px-2 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-400 focus:outline-none pr-7"
            />
            {searchQuery && (
              <button
            ...
            <tbody>
              {rows.filter(row => {
                ...
                <BulkRowEditor
                  ...
                  onOverrideSystemQty={handleOpenOverride}
      ...
      {/* System quantity override modal */}
      {overrideModalRowId && (() => {
        const overrideRow = rows.find(r => r.rowId === overrideModalRowId)
        return (
          <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-black/60" onClick={() => setOverrideModalRowId(null)} />
            <div className="relative bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-amber-300 dark:border-amber-700 w-full max-w-sm p-5">
              <h2 className="text-sm font-bold text-gray-900 dark:text-white mb-0.5">Override System Quantity</h2>
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-4 truncate">{overrideRow?.name}</p>
              <div className="space-y-3">
                <div>
                  <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">
                    New system qty <span className="text-gray-400">(was {overrideRow?.currentStock ?? '—'})</span>
                  </label>
                  <input
                    type="number"
                    min="0"
                    value={overrideQtyInput}
                    onChange={e => setOverrideQtyInput(e.target.value)}
                    autoFocus
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-amber-400 focus:outline-none resize-none"
                    placeholder="Explain why the system quantity needs to be corrected…"
                  />
                </div>
              </div>
              <div className="flex items-center gap-2 mt-4">
                <button
                  ...
                  disabled={!overrideReasonInput.trim() || overrideQtyInput === '' || parseInt(overrideQtyInput, 10) < 0}
                ...
                {overrideRow?.systemQuantityOverride !== null && (
                  <button
                    ...
                    title="Remove override and restore original system qty"
                  >
                    Clear
                  </button>
                )}
                <button
                  onClick={() => setOverrideModalRowId(null)}
                  className="px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg"
                >
                  Cancel
                </button>
...
interface BulkRowEditorProps {
  ...
  onRemove: () => void
  onToggleEdit: () => void
  onOverrideSystemQty: (rowId: string) => void
  onNewCategory: () => void
...
function BulkRowEditor({ row, rowNumber, domains, departments, allCategories, allSubCategories, suppliers, hasDeptCol, hasSubCatCol, hasPhysicalCountCol, invalidFields, onChange, onRemove, onToggleEdit, onOverrideSystemQty, onNewCategory, onNewSubCategory, onNewSupplier, rowRef, isStockTakeMode, isHighlighted, needsReview, transportConfig, totalBatchQuantity }: BulkRowEditorProps) {
  ...
  // Effective system qty — use override if set, otherwise live currentStock
  const effectiveSystemQty = row.systemQuantityOverride !== null ? row.systemQuantityOverride : row.currentStock
  // Variance = physicalCount − systemQuantity (only for existing items)
  const variance = row.isExistingItem && row.physicalCount !== '' && effectiveSystemQty !== null
    ? Number(row.physicalCount) - effectiveSystemQty
    : null
    ...
    <tr ref={rowRef} className={`border-b border-gray-100 dark:border-gray-700 transition-all ${rowStatusClass} ${highlightClass}`}>
      ...
      <td className="px-2 py-1.5 text-center text-xs">
        <div className="flex items-center justify-center gap-1">
          <span className={row.systemQuantityOverride !== null ? 'text-amber-600 dark:text-amber-400 font-semibold' : 'text-gray-500'}>
            {effectiveSystemQty !== null ? effectiveSystemQty : '—'}
          </span>
          {row.systemQuantityOverride !== null && (
            <span title={`Override: ${row.systemOverrideReason}\nBy: ${row.systemOverriddenBy}`} className="text-[9px] text-amber-500 leading-none font-bold cursor-help">⚑</span>
          )}
          {row.isExistingItem && isStockTakeMode && (
            <button
              type="button"
              onClick={() => onOverrideSystemQty(row.rowId)}
              title={row.systemQuantityOverride !== null ? 'Edit system qty override' : 'Override system qty'}
              className="text-[11px] leading-none text-gray-300 hover:text-amber-500 dark:hover:text-amber-400 ml-0.5"
            >✏️</button>
          )}
        </div>
      </td>
