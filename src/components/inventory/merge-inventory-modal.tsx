...
export function MergeInventoryModal({ businessId, products, onClose, onMerged }: Props) {
  // Winner = highest current stock
  ...
  const combinedStock = products.reduce((sum, p) => sum + p.currentStock, 0)
  ...
  const handleConfirm = async () => {
    ...
    setError('')
    try {
      const res = await fetch(`/api/inventory/${businessId}/merge`, {
        method: 'POST',
    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md mx-auto">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-base font-bold text-gray-900 dark:text-white">🔀 Merge Inventory Items</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 text-xl leading-none">&times;</button>
        </div>

        {/* Body */}
        <div className="px-5 py-4 space-y-4">
          <p className="text-sm text-gray-600 dark:text-gray-400">Merge these items into one, combining their stock quantities:</p>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3 px-3 py-2.5 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">{winner.sku ?? '—'} · {winner.currentStock} in stock</p>
              </div>
            </div>
            ...
            {losers.map(l => (
              <div key={l.id} className="flex items-center gap-3 px-3 py-2.5 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">{l.sku ?? '—'} · {l.currentStock} in stock</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-400"
              />
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-gray-600 dark:text-gray-400">Combined stock:</span>
              <span className="font-bold text-gray-900 dark:text-white">{combinedStock} units</span>
            </div>
          </div>

          <p className="text-xs text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-700/50 rounded-lg px-3 py-2">
            The removed item(s) will be deactivated and their stock added to the kept item. This cannot be undone.
          </p>
        ...
        <div className="flex gap-3 px-5 py-4 border-t border-gray-200 dark:border-gray-700">
          ...
          <button
            ...
            className={`flex-1 px-4 py-2 text-sm rounded-lg font-medium text-white disabled:opacity-50 ${confirming ? 'bg-red-600 hover:bg-red-700' : 'bg-indigo-600 hover:bg-indigo-700'}`}>
            {merging ? 'Merging…' : confirming ? 'Confirm & Merge' : 'Merge Items'}
          </button>
        </div>
      </div>
    </div>
