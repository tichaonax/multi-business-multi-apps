...
export function SubcategoryCreator({
  categoryId,
  categoryName,
  categoryEmoji,
  onSuccess,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: CreateSubcategoryRequest = {
        categoryId,
        name: name.trim(),
      ...
      const response = await fetch('/api/expense-categories/subcategories', {
        method: 'POST',
        headers: {
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-6">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g., Mobile Banking Fees"
              required
              disabled={loading}
          ...
          {(name || emoji) && (
            <div className="bg-gray-50 border border-gray-200 rounded p-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium">{name || 'Subcategory name'}</span>
              </div>
              {description && (
