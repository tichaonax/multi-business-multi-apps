...
export async function createCleanBackup(
  ...
  const scaleBusinessFilter = businessId ? { businessId: { in: businessIds } } : {}
  businessData.weightPricingRules = await (prisma as any).weightPricingRules.findMany({
    where: scaleBusinessFilter
  })
  businessData.livestockPurchaseSessions = await (prisma as any).livestockPurchaseSessions.findMany({
    where: scaleBusinessFilter
  })
  ...
  businessData.livestockPurchaseLines = sessionIds.length > 0
    ? await (prisma as any).livestockPurchaseLines.findMany({ where: { sessionId: { in: sessionIds } } })
    : []
  businessData.livestockVendorProfiles = await (prisma as any).livestockVendorProfiles.findMany({
    where: scaleBusinessFilter
  })
