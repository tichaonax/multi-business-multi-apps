...
export async function processRentTransfer(
  ...
  // Rent account is funded at approval time (not daily). Daily EOD only creates
  // a cash bucket earmark via the cash-allocation lock. No expense account deposit here.
  return { alreadyTransferred: false, depositId: '', amount: 0 }
}
