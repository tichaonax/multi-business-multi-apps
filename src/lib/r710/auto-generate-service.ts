...
async function generateTokensForConfig(
  ...
  // Store tokens in database — convert to seconds for validTimeSeconds and to
  // ms for the expiry calc. Uses apiDuration/apiDurationUnit (already
  // week-to-day converted above), not the config's raw stored value, so this
  // stays in sync with what was actually sent to the device.
  const durationSeconds = apiDurationUnit === 'day'
    ? apiDuration * 86400
    : apiDuration * 3600; // 'hour'
