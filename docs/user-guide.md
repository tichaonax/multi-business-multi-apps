...
**A shared printer only routes to one business at a time (MBM-277).** A physical printer (declared on a workstation's row, Section 26) can only be AGENT-relayed through one business's workstation pairing at once — if two businesses run from the same physical till, only whichever business's pairing that printer is currently routed to can print through it; the other gets the "local agent is offline" message above even though the workstation's agent itself is online, because it's simply routed elsewhere right now. **This now re-points itself automatically** whenever the active business changes on that workstation (MBM-280 — see "Multiple Businesses, One Workstation" below); an admin only needs **Workstation Agents** (Section 26) for the very first setup, not to hand the printer between businesses afterward. Each business also keeps its **own** "default printer" preference (**POS Settings → Printer Preferences**) — picking a default printer while working one business no longer carries over as the default when you switch to a different business on the same browser/PC.
...
**The shared printer's routing follows the switch automatically too (MBM-280).** Every time a business becomes active on a workstation, any printer still routed (declared on a workstation's row, Section 26) to one of that *same* workstation's other businesses is automatically re-pointed onto the newly active one — so switching business also switches which business's receipts the shared printer prints, with no manual re-routing step needed after the first setup. This only ever moves a printer that was already following one of this exact workstation's own other businesses; it never touches a printer belonging to an unrelated machine.
...
3. Click **List Printers** to pull the actual printer names Windows sees on that workstation, and pick the correct one — no separate "register a printer" step first, and no picking a workstation from a dropdown, since you're already on that workstation's own row.
...
**Business-wide default** — the final fallback for a device with neither a terminal registration nor a workstation pairing (the common case for a phone that's never been set up specially). Set it either at **Settings → POS Settings → Printer Preferences** (business-owner/admin section, scoped to whichever business you currently have active), or at **Admin → Default Printer** (`/admin/network-printers`) — a system-admin-only page with a business selector, for setting it centrally without needing to switch your active business first. Both write to the exact same setting; either one works.

**Resolution order when a receipt is about to print:** the printer you personally picked on this device (if valid for this business) → **this workstation's own declared printer**, if this machine is a paired workstation with remote printing on → your saved **QZ Tray** printer on this machine, if any → **this device's registered Print Terminal default**, if it's been registered → the **business-wide default** → if none of those apply, you're prompted to choose one manually. A device is only ever one of "registered terminal" or "paired workstation," never both, so only one of those two middle steps ever actually applies.
