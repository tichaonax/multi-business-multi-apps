...
The most common specific cause: the server's certificate was generated for a different IP address/hostname than the one it's actually being reached at now (e.g. the server moved to a new IP, or got a second one added). To fix, on the **application server** (not the workstation):

1. Confirm the mismatch — check the file names in that server's `certs/` folder against the address the workstation is actually connecting to (visible in `serverUrl` for that pairing, or simply the URL you typed in when pairing).
2. Regenerate the certificate to cover the current address(es) — see the **Server-side certificates** section of `docs/TECHNICAL-DEPLOYMENT-GUIDE.md` for the exact `mkcert` commands (Windows and Linux). Regenerating reuses the same root CA, so nothing needs re-trusting on workstations that already trust it.
3. Restart the server so it picks up the new certificate.
4. **Revoke that pairing and re-pair it** — this is the step that's easy to miss. Just restarting the agent is not enough: the bad or missing certificate is already saved in the agent's own pairing file, and only a fresh pairing hands it a corrected one. Re-pairing does not require downloading the agent again (see the Revoke Pairing note above).
