...
| **Business** | Which business is currently **active** for this workstation's scale/printer pairing (workstation profiles only) — see "Multiple Businesses, One Workstation" below if this machine is paired for more than one |
...
A single physical scale and printer can be shared by **several businesses** on the same workstation — common on a shared till that different businesses use at different times. Each business pairs it once (**Pair this machine**, above) the normal way, from its own Workstation Agents page; the workstation keeps every business's pairing it's ever seen, permanently, rather than only remembering whichever one paired most recently.

Only **one business is ever active** for the scale/printer at a time on a given workstation — that's a real hardware fact (one COM port, one physical printer), not a limitation of the app. Which one is active follows you automatically:

- **Switching business from the header dropdown — on the same PC the workstation agent is running on — switches the workstation's active business to match**, instantly. No separate step needed; it happens the moment you pick a different business, from any page.
- **Switching browser/tab focus back to an already-open tab does the same thing (MBM-282), even with no business switch involved.** If you have two tabs open on this exact workstation — say Business A in one tab, Business B in another, whether that's two tabs on the *same* server or, per "One agent, many servers at once" above, two tabs on two *different* paired servers — simply clicking back into whichever tab you want active re-asserts it as the workstation's active business, with no dropdown action needed in that tab. This is what makes the scale in particular follow your attention correctly: previously, only an actual in-tab business *switch* counted, so alt-tabbing between two already-loaded tabs left the workstation pinned to whichever one last performed an explicit switch, not whichever one you'd actually clicked into.
- If the business you switch (or focus) to has never been paired on this exact workstation, its scale/printer is simply cleared/deactivated here — nothing from the previous business is left connected.
...
- Creating a brand-new business and being switched into it automatically works the same way — the new business has no pairing yet, so the workstation's scale/printer is simply deactivated until you pair it there.
- **The receipt printer and R710 never needed any of this switching logic in the first place** — every print job and R710 request is a single, self-contained round trip carrying its own printer name/device details, so two businesses' jobs can already run at the same time with no conflict (the same way two unrelated desktop apps can print to one shared printer without stepping on each other). The scale is the one physical device that's genuinely exclusive (a single serial port only one program can hold open at a time), which is why it's the only one that needs "active business" tracking at all.
...
**A business that isn't currently active here shows the same "local agent is offline" message as a genuinely disconnected agent** if you try to use its scale or printer — this extends the printer-sharing behavior described just above to the scale too. The workstation's agent process is running fine; it's simply connected on behalf of a *different* business right now. Switching to that business (dropdown, tray, or Manage Profiles) resolves it immediately, with no re-pairing needed.

**The printer itself needs no "which business is active" switching at all** — unlike the scale above, each business's printer declaration (Section 26) is independent and always live for that business, regardless of which business is currently active on this workstation for the scale. Switching active business only matters here for the scale; a receipt for either business prints correctly at any time.
...
**Resolution order when a receipt is about to print:** the printer you personally picked on this device (if valid for this business) → **an admin-set override for this specific workstation**, if one exists → **this workstation's own declared printer**, if this machine is a paired workstation with remote printing on → your saved **QZ Tray** printer on this machine, if any → **this device's registered Print Terminal default**, if it's been registered → the **business-wide default** → if none of those apply, you're prompted to choose one manually. A device is only ever one of "registered terminal" or "paired workstation," never both, so only one of those two middle steps ever actually applies.
