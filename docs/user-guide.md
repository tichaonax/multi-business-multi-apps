...
3. Once paired, the page shows two setup cards, one for each capability — use whichever apply to this workstation:
   ...
   - **🖨️ Receipt Printer Setup** — **leads with a status summary, not just instructions**, whenever anything is already set up: a green ✅ block per paired workstation showing exactly what's live right now, e.g. "Agent-relayed: EPSON TM-T (server → this workstation)" and/or "QZ Tray (this machine's browser): EPSON TM-T" — this business's two independent print paths, shown separately since a workstation can have either, both, or neither (see the tray reference table further down for what distinguishes them). Any workstation with nothing configured yet is listed separately underneath, so it's obvious at a glance what still needs attention. Below that summary, the same two-step instructions as before remain available for adding or changing an agent-relayed printer — this card doesn't configure the printer directly (printers are a shared, cross-business resource, handled on two separate admin-only pages — see Section 26's **"Printer Connection Mode"** for the full walkthrough) but gives you direct links to both steps right from here: **Register a printer →** and, for system admins, **Printer Connection Mode →**. If you're not a system admin, it names the workstation(s) you just paired and tells you to hand those off to one. There's also a reminder link to **👤 Profile → Printer Setup** for QZ Tray, since that's configured separately, per browser/machine, not on this page.
   > This summary reads live from the database — it doesn't depend on the agent's live connection or wait for its next periodic sync, so it stays accurate even when a workstation is temporarily offline. Saving a new printer (either path) now also pushes an immediate refresh to that workstation's tray, instead of the tray waiting up to 10 minutes to catch up.
   >
   > **System admins also see a "Printer connection mode & status" list right here (MBM-278)** — every printer registered on the server, its current DIRECT/AGENT mode, and for AGENT mode, which workstation it's actually routed through, that workstation's business, its live 🟢/🔴 status, and a **"— not this business"** flag if it's currently routed to a *different* business's workstation than the one you're viewing. Previously this was only visible by opening the separate **Printer Connection Mode** page directly — a printer silently misrouted to another business's agent (see MBM-277) gave no signal at all on this page until you went looking there. Business owners (non-system-admin) don't see this list, since it spans every business's printers, not just this one — they still see only the current-configuration summary above. Each row has its own **Configure →** link straight to Printer Connection Mode to fix it.
...
**A shared printer only routes to one business at a time (MBM-277).** A physical printer (`Printer Connection Mode`, Section 26) can only be AGENT-relayed through one business's workstation pairing at once — if two businesses run from the same physical till, only whichever business's pairing that printer is currently routed to can print through it; the other gets the "local agent is offline" message above even though the workstation's agent itself is online, because it's simply routed elsewhere right now. **This now re-points itself automatically** whenever the active business changes on that workstation (MBM-280 — see "Multiple Businesses, One Workstation" below); a system admin only needs **Printer Connection Mode** (Section 26) for the very first setup, not to hand the printer between businesses afterward. Each business also keeps its **own** "default printer" preference (**POS Settings → Printer Preferences**) — picking a default printer while working one business no longer carries over as the default when you switch to a different business on the same browser/PC.
...
**The shared printer's routing follows the switch automatically too (MBM-280).** Every time a business becomes active on a workstation, any printer still routed (via **Printer Connection Mode**, Section 26) to one of that *same* workstation's other businesses is automatically re-pointed onto the newly active one — so switching business also switches which business's receipts the shared printer prints, with no manual Printer Connection Mode step needed after the first setup. This only ever moves a printer that was already following one of this exact workstation's own other businesses; it never touches a printer belonging to an unrelated machine.
