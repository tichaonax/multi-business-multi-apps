...
>
> **One caveat if a workstation is paired to more than one server (below) at different versions:** the comparison is an exact match against *that specific server's* currently-built agent version, not a minimum-version check — so a workstation could show "up to date" against one paired server while showing "update available" against another that shipped a newer agent build. Update based on whichever server's panel is showing the warning; the same single `.exe` update satisfies every server it's paired to.
