...
async function createBusinessAccounts(businessId, businessName, creatorId = 'admin-system-user-default') {
  const INCOME_AMOUNT = 15000    // Opening credit on business account (represents accumulated sales)
  const EXPENSE_TRANSFER = 5000  // Transfer from business account → expense account
  ...
  // 2. Create expense account if missing, or link it if businessId was null
  ...
  // 5. Transfer from business account → expense account (BUSINESS-type deposit)
  ...
  if (!existingDeposit) {
    ...
    await prisma.expenseAccountDeposits.create({
      data: {
        ...
        amount: EXPENSE_TRANSFER,
        depositDate: new Date(),
        autoGeneratedNote: `Transfer from ${businessName} for operating expenses`,
        createdBy: creatorId,
      }
    })
    await prisma.businessTransactions.create({
...
async function seed() {
  try {
    ...
    // Create business account and expense account (for existing businesses too)
    await createBusinessAccounts(businessId, business.name)
    ...
    try {
      ...
      if (products.length > 0) {
        ...
        for (const o of sampleOrders) {
          ...
          let order = await prisma.businessOrders.findFirst({ where: { businessId, orderNumber: o.orderNumber } })
          if (!order) {
            order = await prisma.businessOrders.create({ data: { ...orderData, createdAt: new Date(), updatedAt: new Date() } })
          } else {
            await prisma.businessOrders.update({ where: { id: order.id }, data: { ...orderData, updatedAt: new Date() } })
          }
          ...
          const existingTicket = await prisma.businessOrders.findFirst({ where: { businessId, orderNumber: ticketNumber } })
          if (!existingTicket) {
            await prisma.businessOrders.create({ data: { ...ticketData, createdAt: new Date(), updatedAt: new Date() } })
          } else {
            await prisma.businessOrders.update({ where: { id: existingTicket.id }, data: { ...ticketData, updatedAt: new Date() } })
          }
