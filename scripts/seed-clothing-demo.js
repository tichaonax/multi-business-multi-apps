...
async function createBusinessAccounts(businessId, businessName, creatorId = 'admin-system-user-default') {
  const INCOME_AMOUNT = 11000    // Opening credit on business account (represents accumulated sales)
  const EXPENSE_TRANSFER = 3500  // Transfer from business account → expense account
...
async function seed() {
  try {
    ...
    for (const p of products) {
      ...
      // Create variants (findFirst + create to stay idempotent without needing a unique index on sku)
      for (const v of p.variants || []) {
        let variant = await prisma.productVariants.findFirst({ where: { sku: v.sku } })
        if (!variant) {
          variant = await prisma.productVariants.create({
            data: { productId: prod.id, name: v.name, sku: v.sku, barcode: null, price: v.price, stockQuantity: v.stock, isActive: true, createdAt: now, updatedAt: now }
          }).catch(() => null)
        } else {
          await prisma.productVariants.update({ where: { id: variant.id }, data: { price: v.price, stockQuantity: v.stock, isActive: true, updatedAt: now } }).catch(() => null)
        }
