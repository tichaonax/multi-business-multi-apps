...
async function handleCreate(params, next, changeTracker, model) {
...
async function handleUpdate(params, next, changeTracker, model) {
    let beforeData = null;
    try {
        if (params.where) {
            beforeData = await next.prisma[model.toLowerCase()].findUnique({
                where: params.where
            });
        }
    }
...
async function handleUpsert(params, next, changeTracker, model) {
    let existingRecord = null;
    try {
        if (params.where) {
            existingRecord = await next.prisma[model.toLowerCase()].findUnique({
                where: params.where
            });
        }
    }
...
async function handleDelete(params, next, changeTracker, model) {
    let beforeData = null;
    try {
        if (params.where) {
            beforeData = await next.prisma[model.toLowerCase()].findUnique({
...
async function handleCreateMany(params, next, changeTracker, model) {
    const result = await next(params);
    try {
...
async function handleUpdateMany(params, next, changeTracker, model) {
    let affectedRecords = [];
    try {
        if (params.where) {
            affectedRecords = await next.prisma[model.toLowerCase()].findMany({
                where: params.where
            });
        }
    }
...
async function handleDeleteMany(params, next, changeTracker, model) {
    let affectedRecords = [];
    try {
        if (params.where) {
            affectedRecords = await next.prisma[model.toLowerCase()].findMany({
...
async function initializeSyncSystem(prisma, options) {
    try {
        ...
        const changeTracker = (0, change_tracker_1.getChangeTracker)(prisma, options.nodeId, options.registrationKey);
        await changeTracker.initializeNode(options.nodeName, options.ipAddress, options.port || 3001);
