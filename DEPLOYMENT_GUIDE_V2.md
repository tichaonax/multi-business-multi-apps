...
# 4. Run fresh installation (includes automatic category seeding)
npm run setup

# 5. Install and start Windows service (as Administrator)
...
- **Seeds business categories** (20 categories, 59 subcategories across 4 business types)
- Builds the application
- Creates admin user: `admin@business.local` / `admin123`

**✅ Categories Seeded Automatically:**
- **Clothing**: 5 categories, 17 subcategories (Men's, Women's, Kids Fashion, Footwear, Accessories)
- **Hardware**: 5 categories, 14 subcategories (Hand Tools, Power Tools, Building Materials, Plumbing, Electrical)
- **Grocery**: 6 categories, 15 subcategories (Produce, Meat & Seafood, Dairy, Bakery, Beverages, Pantry)
- **Restaurant**: 4 categories, 13 subcategories (Appetizers, Main Courses, Desserts, Beverages)

**✅ No Manual Database Creation or Category Seeding Required!**
The setup script automatically handles database creation, migrations, and category seeding
...
- [ ] `npm run setup` completed successfully (includes automatic category seeding)
...
### Verify Categories Were Seeded (Optional)

Run this SQL query to confirm all 20 categories were seeded:
...
**Note**: Categories are now automatically seeded during `npm run setup`. Manual seeding is no longer required.
